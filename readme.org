...
#+BEGIN_SRC emacs-lisp 
  (defun jmqu/test-backward-kill-word ()
    "Smart kill word backward."
    (interactive)
    (backword-char))
  (global-set-key [C-backspace]
                  'jmqu/test-backward-kill-word)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp 
  (defun jmqu/backward-kill-word ()
    "Smart kill word backward."
    (interactive)
    (let* ((cp (point)) ;; save current point
           (backword)   ;; define 
           (end)        ;; define 
           (space-pos)  ;; define
           (backword-char (if (bobp) ;; save the character before current point
                              "" ;; if buffer beginning
                            (buffer-substring cp (- cp 1)))))
      (if (equal (length backword-char) (string-width backword-char)) ;; if it is an Ascii character
          (progn
            (save-excursion
